class Render {
    ...
    OutFilmData(item, favoryte = 0) {
        var tmp = '';
        var idFilm = item['kinopoiskId'];
        if (!idFilm) idFilm = item['filmId'];

        tmp += '<div class="film-actions">'; // обёртка для всех кнопок

        tmp += '<a href="persons.html?id=' + idFilm + '" ><img src=\'img/artist.png\' class=\'bottons rating-button\'></a>' +
            '<a href="similar.html?id=' + idFilm + '" ><img src=\'img/similar.png\' class=\'bottons rating-button\'></a>' +
            '<a href="equel.html?id=' + idFilm + '"><img  src=\'img/equel.png\' class=\'bottons rating-button\'></a>' +
            '<a href="img.html?id=' + idFilm + '"><img  src=\'img/img.png\' class=\'bottons rating-button\'></a>' +
            "<a href='torrent.html?q=" + this.clearStr(item['nameRu']) + "&e=" + this.clearStr(item['nameOriginal']) + "&year=" + item['year'] + "&id=" + idFilm + "'><img  src='img/torrent.jpg' class='bottons rating-button'></a>" +
            "<img src='img/like.png' class='bottons rating-button' data-id='" + idFilm + "' onclick='showRatingWindow(" + idFilm + ")' style='cursor:pointer;' title='Оценить фильм'>";
        ...
        tmp += '</div>'; // закрываем обёртку

        tmp += "<hr><a href='https://www.kinopoisk.ru/film/" + idFilm + "/' target='_blank'>Кп:" + item['ratingKinopoisk'] + "</a> <a href='https://www.imdb.com/find/?q=" + this.clearStr(item['nameOriginal'] || item['nameRu']) + "' target='_blank'>IMDb:" + item['ratingImdb'] + "</a> Крит.: " + Number(item.ratingFilmCritics) + "<ul><li>" + item['year'] + "г.</li><li>" + item['filmLength'] + " мин.</li><li>" + item['countries'] + "</li><li>" + item['genres'] + "</li></ul>" + item['description'];
        return tmp;
    }
